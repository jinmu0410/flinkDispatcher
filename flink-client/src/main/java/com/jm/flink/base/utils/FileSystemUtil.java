...
public class FileSystemUtil {
    ...
    private static final String KEY_HADOOP_SECURITY_AUTHORIZATION_TYPE_KERBEROS = "KERBEROS";
    private static final String KEY_HADOOP_SECURITY_AUTHORIZATION_TYPE_TBDS = "TBDS";


    private static final String KEY_HADOOP_SECURITY_AUTHORIZATION_TYPE = "hadoop.security.authorization.type";
    ...
    public static FileSystem getFileSystem(
            ...
            throws Exception {
        if (isOpenKerberos(hadoopConfigMap)) {
            return getFsWithKerberos(hadoopConfigMap);
        }

        Configuration conf = getConfiguration(hadoopConfigMap, null);
        setHadoopUserName(conf);

        return FileSystem.get(getConfiguration(hadoopConfigMap, null));
    }
    ...
    private static FileSystem getFsWithKerberos(
            ...
            throws Exception {
        UserGroupInformation ugi = getUGI(hadoopConfig);

        return ugi.doAs(
                (PrivilegedAction<FileSystem>)
                        () -> {
                            try {
                                return FileSystem.get(getConfiguration(hadoopConfig, null));
                            } catch (Exception e) {
                                throw new RuntimeException(
                                        "Get FileSystem with kerberos error:", e);
                            }
    ...
    public static UserGroupInformation getUGI(
            Map<String, Object> hadoopConfig)
            throws IOException {
        String keytabFilePath = KerberosUtil.getPrincipalFileName(hadoopConfig);
        keytabFilePath = KerberosUtil.loadFile(hadoopConfig, keytabFilePath, null);
        String principal = KerberosUtil.getPrincipal(hadoopConfig, keytabFilePath);
        KerberosUtil.loadKrb5Conf(hadoopConfig, null);
        KerberosUtil.refreshConfig();

        // Krb5Conf已经LOAD 不需要重新RELOAD
        return KerberosUtil.loginAndReturnUgi(principal, keytabFilePath, null);
    }
    ...
    public static UserGroupInformation getUGI(
            ...
            throws IOException {
        String keytabFilePath = KerberosUtil.getPrincipalFileName(hadoopConfig);
        keytabFilePath = KerberosUtil.loadFile(hadoopConfig, keytabFilePath, distributedCache);
        String principal = KerberosUtil.getPrincipal(hadoopConfig, keytabFilePath);
